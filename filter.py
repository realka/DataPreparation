...
for ad in subfolder:
    for x in range(350):
        ...
        image = cv2.imread("Train/"+ad+"/"+ad+"."+str(x+1)+".jpg")
        #Median#
        #Median#median = cv2.medianBlur(image,5)
        #Median#plt.imsave("Mediantrain/"+ad+"/"+ad+"."+str(x+1)+".jpg", median,cmap = 'gray')
        ...
        #Gabor#filtered = filtered.astype(np.float32)
        #Gabor#status = cv2.imwrite("Gtrain/"+ad+"/"+ad+"."+str(x+1)+".jpg", filtered)
        #Gabor#print(status)

        #bilateralFilter
        #bilateralFilter#bilateral = cv2.bilateralFilter(image,5,150,150)
        #bilateralFilter#image = cv2.addWeighted(image, 1.5, bilateral, -0.5, 0)
        #bilateralFilter#image = cv2.bilateralFilter(image, 5, 150, 150)
        #bilateralFilter#status = cv2.imwrite("Bilateraltrain/"+ad+"/"+ad+"."+str(x+1)+".jpg", image)

        #blur
        #blur# blur = cv2.blur(image,(5,5))
        #blur# status = cv2.imwrite("Blurtrain/"+ad+"/"+ad+"."+str(x+1)+".jpg", blur)

        #gaussianBlur
        #gaussianBlur# gblur = cv2.GaussianBlur(image,(5,5),0)
        #gaussianBlur# status = cv2.imwrite("GaussianBlurtrain/"+ad+"/"+ad+"."+str(x+1)+".jpg", gblur)
        ...
        # status = cv2.imwrite("Boxedtrain/"+ad+"/"+ad+"."+str(x+1)+".jpg", image)

        #erosion
        # kernel = np.ones((2,2),np.uint8)
        # erosion = cv2.erode(image,kernel,iterations = 1) # fosilleri ve delikleri buluyor
        # dilation = cv2.dilate(erosion,kernel,iterations = 1)  #damarları buluyor
        # opening = cv2.morphologyEx(image, cv2.MORPH_OPEN, kernel) # fosilleri ve delikleri buluyor
        # closing = cv2.morphologyEx(image, cv2.MORPH_CLOSE, kernel) #damarları buluyor
        # gradient = cv2.morphologyEx(image, cv2.MORPH_GRADIENT, kernel)
        # tophat = cv2.morphologyEx(image, cv2.MORPH_TOPHAT, kernel)
        #blackhat = cv2.morphologyEx(image, cv2.MORPH_BLACKHAT, kernel)
        # status = cv2.imwrite("2x2Erosion+Dilation_Train/"+ad+"/"+ad+"."+str(x+1)+".jpg", dilation)
        # status = cv2.imwrite("4x4Dilationtrain/"+ad+"/"+ad+"."+str(x+1)+".jpg", dilation)
        # status = cv2.imwrite("4x4Openingtrain/"+ad+"/"+ad+"."+str(x+1)+".jpg", opening)
        # status = cv2.imwrite("4x4Closingtrain/"+ad+"/"+ad+"."+str(x+1)+".jpg", closing)

        # sepFilter2D
        # kernel = cv2.getGaussianKernel( 15, 2 )
        # twoD = cv2.sepFilter2D(image, -1, kernel, kernel)
        # status = cv2.imwrite("sepFiltertrain/"+ad+"/"+ad+"."+str(x+1)+".jpg", twoD)
